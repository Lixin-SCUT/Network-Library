...
const char* strerrnor_tl(int savedErrno)
{	return strerror_r(savedErrno, t_errnobuf, sizeof(t_errnobuf)); }

Logger::LogLevel initLogLevel()
{
	if(::getenv("MAIN_LOG_TRACE"))
	{	return Logger::TRACE; }
	else if(::getenv("MAIN_LOG_DEBUG"))
	{	return Logger::DEBUG; }
	else
	{	return Logger::INFO; }
}
...
Logger::Impl::Impl(LogLevel levelm,
				   int savedErrno, 
				   cosnt SourceFile& file,
				   int line)
	:	time_(Timestamp::now()),
		stream_(),
		level_(level),
...
Logger::Logger(SourceFile file, int file)
